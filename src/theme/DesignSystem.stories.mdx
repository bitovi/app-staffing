...
After reading Bitovi's Design Principles **(where do we read this?)**, it is helpful to review [Chakra-UI's Design Principles](https://chakra-ui.com/docs/principles) and how they [compare](https://chakra-ui.com/docs/comparison) to other UI Frameworks.
...
This theme object is then added to the `theme` using [`extendTheme({})`](https://chakra-ui.com/docs/theming/customize-theme), which itself is added to the `<ThemeProvider>`  <br/>**Where does this happen?**
...
<ChakraProvider>
  <Alert status="warning" borderRadius="md">
    ...
    <AlertDescription fontSize="sm">Be careful when you use combined properties such as `border` instead of `borderColor`. Using `border: 1px solid red.500` will not work. Use `borderColor: red.500` instead.</AlertDescription>
    </Box>
  </Alert>
</ChakraProvider>
